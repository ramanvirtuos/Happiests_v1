...
class Mahana_messaging
{
    public function __construct()
    {
        ...
        $this->ci->load->model('message_model','mahana_model');
    ...
    function get_message_attachment($msg_id){
        ...
        if ($attachment = $this->ci->mahana_model->get_message_attachment($msg_id))
        {
            return $this->_success($attachment);
        }
    ...
    function get_msg_attachment($thread_id, $user_id){
        ...
        if ($attachment = $this->ci->mahana_model->get_msg_attachment($thread_id, $user_id))
        {
            return $this->_success($attachment);
        }
     ...
     * @return  array
     */
    function get_all_receive_threads($user_id, $full_thread = FALSE, $order_by = 'ASC',$limilt_per_page =NULL,$offset = NULL)
    {
        if (empty($user_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_USER_ID);
        ...
        $message = $this->ci->mahana_model->get_all_receive_threads($user_id, $full_thread, $order_by,$limilt_per_page,$offset);
        if (is_array($message))
        {
            return $this->_success($message);
        }
    ...
    /**
     * get_all_sent_threads() - will return Sent messages for user, including the status for specified user.
     *
     * @param   integer  $user_id      REQUIRED
     * @param   boolean  $full_thread  OPTIONAL - If true, user will also see messages from thread posted BEFORE user became participant
     * @param   string   $order_by     OPTIONAL
    ...
    function get_all_sent_threads($user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
        ...
        $message = $this->ci->mahana_model->get_all_sent_threads($user_id, $full_thread, $order_by);
        if (is_array($message))
        {
            return $this->_success($message);
        }
    ...
    /**
     * _update_msg_statuses() - removes user from existing thread
     *
    ...
    function _update_msg_statuses($message_id, $user_id)
    {
        if (empty($message_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_MSG_ID);
        ...
        if ($this->ci->mahana_model->_update_msg_statuses($message_id, $user_id))
        {
     ...
     * @return  array
     */
    function send_new_message($sender_id, $recipients, $subject = '', $body = '', $priority = PRIORITY_NORMAL,$attachment=array())
    {
        if (empty($sender_id))
        {
        ...
        if ($thread_id = $this->ci->mahana_model->send_new_message($sender_id, $recipients, $subject, $body, $priority,$attachment))
        {
     ...
     * @return  array
     */
    function reply_to_message($msg_id, $sender_id, $recipients, $body = '', $priority = PRIORITY_NORMAL,$attachment,$participant)
    {
        if (empty($sender_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_SENDER_ID);
        ...
        if ($new_msg_id = $this->ci->mahana_model->reply_to_message($msg_id, $sender_id, $recipients, $body, $priority = PRIORITY_NORMAL,$attachment,$participant))
        {
    ...
    function get_total_msg_count($user_id, $status_id = MSG_STATUS_ARCHIVED)
    {
        ...
        if (is_numeric($message = $this->ci->mahana_model->get_total_msg_count($user_id, $status_id)))
        {
            return $this->_success($message);
        }
